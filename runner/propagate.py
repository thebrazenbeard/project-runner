...
from .currentness import subject_changed
...
def _logical_key(observation: Observation) -> tuple[str, str, str, str | None]:
...
def derive_invalidations(
    ...
    previous_by_key = {_logical_key(item): item for item in previous_observations}
    edge_tuple = tuple(edges)
    result: list[Invalidation] = []

    for current in current_observations:
        previous = previous_by_key.get(_logical_key(current))
