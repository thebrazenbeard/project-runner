...
Lifecycle updates use generation compare-and-swap and an explicit transition map. Entering `CLAIMED`, `RUNNING`, or `VERIFYING` requires the exact current lease holder/fencing token and an unexpired lease in the same SQLite transaction. Generation knowledge alone cannot make a caller the active owner, and lease ownership does not permit lifecycle skips or backward transitions.
